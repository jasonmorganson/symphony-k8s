...
[[ "$rendered_recovery_ids" == "$expected_recovery_ids" ]] || {
  echo "runtime workflow must render the exact frozen-cohort recovery allowlist" >&2
  exit 1
}
grep -A5 '^  terminal_states:$' "$TEMP_DIR/runtime-workflow.md" |
  grep -Fq '    - Done'
