...
grep -q 'do not hold the agent turn open with `gh run watch`' "$throughput_overlay"
grep -q 'call `symphony_report_turn_outcome` with outcome `defer`' "$throughput_overlay"
grep -q 'Symphony will revisit the issue on a later rate-limited turn' "$throughput_overlay"
