...
grep -q 'required hosted job that was skipped lacks a durable exact-command local receipt' \
  "$throughput_overlay"
grep -q '^# A-295 skipped Test(App) recovery proof$' "$throughput_overlay"
grep -q 'TURBO_SCM_BASE=refs/remotes/ci/pr-event-base' "$throughput_overlay"
grep -q 'apps/gateway/src/__tests__/delegation-chain.test.ts' "$throughput_overlay"
