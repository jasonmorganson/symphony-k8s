...
- `SYMPHONY_WORKER_AFFINITY_SEED_FILE` optionally points to a prevalidated
  version-1 issue-to-host registry for the first affinity-aware rollout. The
  deployer writes it atomically before restarting the orchestrator and refuses
  to overwrite existing durable affinity state.

For the migration rollout, generate the seed from immediately revalidated
workspace evidence and run `scripts/seed-worker-affinities.sh SEED_FILE` before
merging. The script validates every host against live Symphony state and will
not replace an existing registry.
