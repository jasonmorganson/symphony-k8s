...
- `vercel-autographing`
  - `VERCEL_TOKEN` scoped to the `autographing` Vercel team; create this Secret
    out of band and never commit the token
...
Workers that need Vercel control-plane access receive a separately scoped
credential:

```bash
read -rs VERCEL_AUTOGRAPHING_TOKEN
printf '\n'
printf '%s' "$VERCEL_AUTOGRAPHING_TOKEN" | \
  KUBECONFIG="$HOME/.kube/symphony-doks.yaml" \
  kubectl -n symphony create secret generic vercel-autographing \
    --from-file=VERCEL_TOKEN=/dev/stdin \
    --dry-run=client -o yaml | \
  KUBECONFIG="$HOME/.kube/symphony-doks.yaml" \
  kubectl apply --server-side -f -
unset VERCEL_AUTOGRAPHING_TOKEN
```

The token is limited to the `autographing` team and is exposed only to worker
pods. Vercel's native Git integration remains the deployment path; the token
is for explicit control-plane operations and does not add a GitHub Actions
deployment.
