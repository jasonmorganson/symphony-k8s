...
`k8s/digitalocean` is the durable single-node k3s overlay. It deliberately
stays separate from the Namespace deployment configuration:

- the dashboard remains a private `ClusterIP` service; the public
  `LoadBalancer` and `Ingress` resources are deleted
- one worker is used on the default 2-vCPU/4-GB Droplet
- orchestrator and worker workspaces use the k3s `local-path` storage class
- the orchestrator and worker images resolve to the node-local `:do` images

Install k3s as a system service with encrypted Kubernetes secret storage:

```bash
curl -sfL https://get.k3s.io | \
  INSTALL_K3S_EXEC="server --disable traefik --write-kubeconfig-mode 600 --secrets-encryption" \
  sh -
```

Build the images on the amd64 Droplet, import the runtime images into k3s,
generate the ignored Skaffold inputs, and deploy the overlay:

```bash
docker build -t symphony-runtime-base:do -f docker/runtime-base/Dockerfile .
docker build -t symphony-release:do -f docker/release/Dockerfile .
docker build \
  --build-arg RUNTIME_BASE=symphony-runtime-base:do \
  --build-arg SYMPHONY_RELEASE_IMAGE=symphony-release:do \
  -t symphony-orchestrator:do -f docker/orchestrator/Dockerfile .
docker build \
  --build-arg RUNTIME_BASE=symphony-runtime-base:do \
  -t symphony-worker:do -f docker/worker/Dockerfile .
docker save symphony-orchestrator:do symphony-worker:do | k3s ctr images import -

bash scripts/generate-skaffold-inputs.sh
k3s kubectl apply -k k8s/digitalocean
rm -rf k8s/base/generated
```

Never commit or retain `k8s/base/generated`: it temporarily contains plaintext
secret inputs. k3s must report `Encryption Status: Enabled` before applying
them. Remove the generated directory immediately after the Kubernetes Secrets
have been created.

Access the private dashboard over SSH:

```bash
ssh -L 4000:127.0.0.1:4000 root@DROPLET_IP \
  'k3s kubectl -n symphony port-forward svc/symphony-orchestrator 4000:4000'
```

Then open <http://127.0.0.1:4000>. The k3s service is enabled at boot and
Kubernetes restarts failed pods automatically. The local-path PVCs survive pod
restarts and redeployments, but they remain tied to this Droplet; include
`/var/lib/rancher/k3s/storage` in host backups.
